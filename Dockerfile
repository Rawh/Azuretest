# FROM mcr.microsoft.com/azure-cli
FROM ubuntu:22.04

# Set environment variables to avoid user prompts during the installation
ENV DEBIAN_FRONTEND=noninteractive

RUN apt update \ 
    && apt install -y wget openssh-server sudo curl iputils-ping dnsutils

# install Azure-CLI according to https://learn.microsoft.com/en-us/cli/azure/install-azure-cli-linux?pivots=apt
RUN curl -sL https://aka.ms/InstallAzureCLIDeb | bash

# Create the required directories and set permissions
RUN mkdir -p /run/sshd \
    && chmod 0755 /run/sshd \
    && mkdir -p /var/run/sshd \
    && chmod 0755 /var/run/sshd

# Add user with public key login
RUN useradd -m -s /bin/bash customuser \
    && mkdir -p /home/customuser/.ssh \
    && mkdir -p /home/customuser/tmp \
    && wget -O /home/customuser/tmp/az.completion "https://raw.githubusercontent.com/Azure/azure-cli/dev/az.completion" \
    && echo "ssh-rsa  customuser@Odin" > /home/customuser/.ssh/authorized_keys \
    && echo "ssh-rsa  customuser@customer" >> /home/customuser/.ssh/authorized_keys \
    && chown -R customuser:customuser /home/customuser/.ssh /home/customuser/tmp \
    && chmod 700 /home/customuser/.ssh /home/customuser/tmp \
    && chmod 600 /home/customuser/.ssh/authorized_keys \
    && echo "source /home/customuser/tmp/az.completion" >> /home/customuser/.bashrc


# Add user to SUDOers 
RUN echo 'customuser ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers

USER root

EXPOSE 22

CMD ["/usr/sbin/sshd", "-D"]
# CMD ["ping", "-4", "localhost"]